...
function Busylight(options) {
	...
	this.getColorArray = function (colors) {
		if (!Array.isArray(colors)) colors = [colors ? getRGB(colors) : this._defaults.color, [0, 0, 0]]
		else
			colors = colors.map(function (color) {
				return getRGB(color)
			})

		if (colors.length == 1) colors.push([0, 0, 0])
...
Busylight.prototype.connect = function (options) {
	if (!options && this.connected) return

	if (this.debug) console.log('Connecting ' + this.connectAttempts)
	...
	if (!device || !device.path) {
		...
		this.reconnectTimer = setTimeout(
			...
			~~Math.pow(1000, (this.connectAttempts > 15 ? 15 : this.connectAttempts) / 10),
		)
		return
	}
...
Busylight.prototype.send = function (p) {
	...
	if (!this.device) {
		if (this.debug) console.log('No device defined')
		return
	}
	...
	if (p && p.tone === false) this.buffer[positions.sound] = 128
	else if (p && p.tone !== undefined)
		this.buffer[positions.sound] = (tones[p.tone] || this._defaults.tone) + p.volume

	if (this.debug) console.info('Writing buffer: ' + this.buffer)

	var checksum
	if (this.newProtocol) {
		checksum = this.buffer.slice(0, 63).reduce(function (a, b) {
			return a + b
		})
...
Busylight.prototype.pulse = function (colors, rate) {
	...
	this.stepper = new Stepper(
		...
		function tick(index) {
			...
			this.send({
				color: tweenRGB(start, end, (1 / (rate / refreshRate)) * (index % (rate / refreshRate))),
			})
		}.bind(this),
	)
